...
import { History, Trophy, Calendar, Coffee, Filter } from 'lucide-react';
...
export default function AdminHistoryPage() {
  ...
  const filteredMatches =
    selectedTableFilter === 'ALL'
      ? historyMatches
      : historyMatches.filter((m) => m.tableNumber === selectedTableFilter);
    ...
    <div className="space-y-6 font-sans">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4 font-mono">
        <div>
          <h1 className="text-xl font-extrabold text-white flex items-center gap-2 font-display">
            ...
            ARSIP RIWAYAT PERTANDINGAN CAFE
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Riwayat lengkap seluruh sesi pertandingan warkop/cafe yang tersimpan di PostgreSQL.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            onChange={(e) => setSelectedTableFilter(e.target.value === 'ALL' ? 'ALL' : Number(e.target.value))}
      ...
      {isLoading ? (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center space-y-3 font-mono">
          ...
          <p className="text-xs text-slate-400">Memuat arsip pertandingan cafe...</p>
        </div>
              ...
              <div key={m.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
                <div className="flex flex-wrap items-center justify-between border-b border-slate-800 pb-3 gap-2">
                  ...
                  <div className="flex items-center gap-3 text-xs text-slate-400">
                    <span>MODE: <strong className="text-emerald-400 uppercase">{m.matchMode}</strong></span>
                    <span>•</span>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1">
                  {sortedPlayers.map((p: any, idx: number) => (
                    <div key={p.id || idx} className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-xs">
                      ...
                      <span className="text-xs font-black text-emerald-400 block mt-0.5">{p.currentScore} Poin</span>
                    </div>
                  ))}
                </div>
              </div>
