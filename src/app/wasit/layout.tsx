...
export default function WasitLayout({ children }: { children: React.ReactNode }) {
  ...
  const wasitLinks = [
    { href: '/wasit/live', label: 'LIVE WASIT', icon: Radio },
    { href: '/wasit/setup', label: 'SETUP MEJA', icon: SlidersHorizontal },
    { href: '/wasit/audit', label: 'AUDIT RONDE', icon: FileText },
    { href: '/wasit/history', label: 'RIWAYAT', icon: History },
  ];

  // Apply zero padding for landscape app-like experience on /wasit/live
  const isLivePage = pathname === '/wasit/live';

  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-cyan-500 selection:text-slate-950">
      {/* Single Wasit Top Header */}
      <header className="sticky top-0 z-40 bg-slate-900/95 backdrop-blur-md border-b border-slate-800 px-4 py-2.5 font-sans">
          ...
          <Link href="/wasit/live" className="flex items-center gap-3 group">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-extrabold text-sm text-white tracking-wide font-display">SIREDOM</span>
              ...
              <p className="text-[10px] text-slate-400 font-mono mt-0.5">
                {tenantCode || 'TAB-SLOWBAR'} • MEJA #{tableNumber || 1}
              </p>
            </div>
          </Link>
        ...
        {/* Mobile Portrait Slide-Out Drawer Menu */}
        {isMobileMenuOpen && (
          <div className="md:hidden bg-slate-950 border-b border-slate-800 p-4 space-y-2 font-mono text-xs animate-in slide-in-from-top duration-200 mt-2">
            {wasitLinks.map((link) => {
              const Icon = link.icon;
              const isActive = pathname === link.href;
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-2.5 px-4 py-3 rounded-xl font-bold transition-all ${
                    isActive
                      ? 'bg-cyan-500 text-slate-950 font-extrabold shadow-md shadow-cyan-500/20'
            ...
            <div className="pt-2 border-t border-slate-800">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-rose-950/80 border border-rose-800 text-rose-300 font-extrabold"
              >
                <LogOut className="w-4 h-4" /> KELUAR SESI WASIT
              </button>
            </div>
          </div>
        )}
      ...
      {/* Main Content Area */}
      <main className={`flex-1 w-full ${isLivePage ? 'p-0 overflow-hidden' : 'max-w-7xl mx-auto p-4 md:p-6'}`}>
        {children}
      </main>
    </div>
  );
